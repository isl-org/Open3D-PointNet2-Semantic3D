...
if STATS:
    ...
    for f, filename in enumerate(filenamesForExport):
        ...
        plt.draw()
        plt.savefig(
            os.path.join(OUTPUT_DIR_HIST, "proba_scene_" + str(f) + ".png"))
    if DRAW_PLOTS:
        plt.show()

    # exporting the probability of being selected and the point seeds.
    print("exporting {} point clouds with densities".format(
        len(filenamesForExport)))
    for f, filename in enumerate(filenamesForExport):
        ...
        np.savetxt(
            os.path.join(
                OUTPUT_DIR_STATS, "{}_{}_{}.txt".format(
        ...
        np.savetxt(
            os.path.join(
                OUTPUT_DIR_STATS, "{}_{}_{}.txt".format(
                    SET, os.path.basename(filename),
                    "appearance_density_cleaned")),
            np.hstack((point_cloud, density.reshape((-1, 1))))[mask],
            delimiter=" ")
        # now the seeds
        np.savetxt(
            os.path.join(
                OUTPUT_DIR_SEEDS, "{}_{}_{}.txt".format(
                    SET, os.path.basename(filename), "seeds")),
        ...
        for j, scene in enumerate(data):
            ...
            if PARAMS['use_color']:
                positioned_data.append(
                    scene + np.array([x, y, z, 0, 0, 0]) -
                    np.min(scene, axis=0) * np.array([1, 1, 1, 0, 0, 0]))
            else:
                positioned_data.append(scene + np.array([x, y, z]) -
                                       np.min(scene, axis=0))
        ...
        for j, scene in enumerate(data):
            ...
            if PARAMS['use_color']:
                pc_util.write_ply_true_color(
                    ...
                    OUTPUT_DIR_BATCHES + "/{}_{}_{}.txt".format(
                        SET, "trueColors", j))
            else:
                pc_util.write_ply_true_color(
