...
parser.add_argument(
    ...
    type=bool,
    default=False,
    help="Stats visualisation on inputs [default: False]",
)
parser.add_argument(
    "--draw",
    type=bool,
    default=True,
    help="Plots in stats visualisation [default: True]",
)
parser.add_argument(
    "--n",
    type=int,
    default=8,
    help="Number of batches you want to visualise [default : 1]",
)
parser.add_argument(
    "--nps",
    type=int,
    default=100,
    help="Number of inputs per scene when doing stats [default : 100]",
)
parser.add_argument(
    "--batch_size", type=int, default=8, help="Batch Size [default: 32]"
)
parser.add_argument(
    "--num_point", type=int, default=4096, help="Point Number [default: 8192]"
)
parser.add_argument(
    "--dataset", default="semantic", help="Dataset [default: semantic_color]"
)
parser.add_argument(
...
if STATS:
    ...
    """
    if not os.path.exists(OUTPUT_DIR_HIST):
        os.mkdir(OUTPUT_DIR_HIST)
    if not os.path.exists(OUTPUT_DIR_STATS):
        os.mkdir(OUTPUT_DIR_STATS)
    if not os.path.exists(OUTPUT_DIR_SEEDS):
        os.mkdir(OUTPUT_DIR_SEEDS)
    ...
    hist_list = (
        list()
    )  # counts for every point of every scene the nb of times it is taken as input
    seeds_list = list()
    ...
    for i in range(STAT_INPUT_NB * len(filenames)):
        ...
        f, input_mask, input_label_hist, seed_idx = DATA.next_input(
    ...
    for f, filename in enumerate(filenamesForExport):
        ...
        plt.ylabel("Proportion")
        plt.title(
            "Histogram of selection likelihood in scene {} of set {}".format(
                filename, SET
            )
        )
        plt.draw()
    ...
    for f, filename in enumerate(filenamesForExport):
        ...
        np.savetxt(
            os.path.join(
                OUTPUT_DIR_STATS,
                "{}_{}_{}.txt".format(
                    SET, os.path.basename(filename), "appearance_density"
                ),
            ),
            np.hstack((point_cloud, density.reshape((-1, 1)))),
            delimiter=" ",
        )
        ...
        np.savetxt(
            os.path.join(
                OUTPUT_DIR_STATS,
                "{}_{}_{}.txt".format(
                    SET, os.path.basename(filename), "appearance_density_cleaned"
                ),
            ),
            np.hstack((point_cloud, density.reshape((-1, 1))))[mask],
            delimiter=" ",
        )
        # now the seeds
        np.savetxt(
            os.path.join(
                OUTPUT_DIR_SEEDS,
                "{}_{}_{}.txt".format(SET, os.path.basename(filename), "seeds"),
            ),
            np.array(seeds_list[f]),
            delimiter=" ",
        )

if NBATCH > 0:
    if not os.path.exists(OUTPUT_DIR_BATCHES):
        os.mkdir(OUTPUT_DIR_BATCHES)
        ...
        for j, scene in enumerate(data):
            ...
            if PARAMS["use_color"]:
                positioned_data.append(
                    scene
                    + np.array([x, y, z, 0, 0, 0])
                    - np.min(scene, axis=0) * np.array([1, 1, 1, 0, 0, 0])
                )
            else:
                positioned_data.append(
                    scene + np.array([x, y, z]) - np.min(scene, axis=0)
                )
        ...
        for j, scene in enumerate(data):
            ...
            if PARAMS["use_color"]:
                pc_util.write_ply_true_color(
                    scene[:, 0:3],
                    (255 * scene[:, 3:6]).astype(int),
                    OUTPUT_DIR_BATCHES + "/{}_{}_{}.txt".format(SET, "trueColors", j),
                )
            else:
                pc_util.write_ply_true_color(
                    scene[:, 0:3],
                    np.zeros(scene[:, 0:3].shape),
                    OUTPUT_DIR_BATCHES + "/{}_{}_{}.txt".format(SET, "trueColors", j),
                )
...
if GROUP_BY_BATCHES and NBATCH > 0:
    if PARAMS["use_color"]:
        pc_util.write_ply_true_color(
            visu_cloud[:, 0:3],
            (255 * visu_cloud[:, 3:6]).astype(int),
            OUTPUT_DIR_BATCHES + "/{}_{}.txt".format(SET, "trueColors"),
        )
    else:
        pc_util.write_ply_true_color(
            visu_cloud[:, 0:3],
            np.zeros(visu_cloud[:, 0:3].shape),
            OUTPUT_DIR_BATCHES + "/{}_{}.txt".format(SET, "trueColors"),
        )
    pc_util.write_ply_color(
        visu_cloud[:, 0:3],
