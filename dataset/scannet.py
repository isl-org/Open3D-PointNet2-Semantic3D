...
class Dataset:
    ...
    def __init__(
        self, npoints, split, use_color, box_size, path, dropout_max, accept_rate
    ):
        if use_color:
            print(
    ...
    def __getitem__(self, index, visu=False):
        ...
        for _ in range(10):
            ...
            mask = (
                np.sum(
                    (cur_point_set >= (curmin - 0.01))
                    * (cur_point_set <= (curmax + 0.01)),
                    axis=1,
                )
                == 3
            )
            vidx = np.ceil(
                (cur_point_set[mask, :] - curmin)
                / (curmax - curmin)
                * [31.0, 31.0, 62.0]
            )
            vidx = np.unique(vidx[:, 0] * 31.0 * 62.0 + vidx[:, 1] * 62.0 + vidx[:, 2])
            isvalid = (
                np.sum(cur_semantic_seg > 0) / len(cur_semantic_seg) >= 0.7
        ...
        if visu:
            return (
                index,
                curchoice,
                np.histogram(cur_semantic_seg, range(22))[0],
                seed_idx,
            )
