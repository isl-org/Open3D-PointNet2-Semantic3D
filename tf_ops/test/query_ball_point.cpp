...
void query_ball_point_cpu(int b, int n, int m, float radius, int nsample,
    ...
    for (int i = 0; i < b; ++i) {
        for (int j = 0; j < m; ++j) {
            ...
            for (int k = 0; k < n; ++k) {
                ...
                float z1 = xyz1[k * 3 + 2];
                float d =
                    max(sqrtf((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1) +
                              (z2 - z1) * (z2 - z1)),
                        1e-20f);
                if (d < radius) {
                    if (cnt == 0) {  // set ALL indices to k, s.t. if there are
                                     // less points in ball than nsample, we
                                     // still have valid (repeating) indices
...
void group_point_cpu(int b, int n, int c, int m, int nsample,
...
void group_point_grad_cpu(int b, int n, int c, int m, int nsample,
...
int main() {
    ...
    for (int i = 0; i < b * n * c; i++) points[i] = randomf();
