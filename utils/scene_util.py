...
#input : matrice avec une ligne par point et 3 colonnes : x,y,z
#en coordonnees cartesiennes
#output : les memes points mais en coordonnees spheriques
def cart2sph(xyz):
    xy = xyz[:, 0]**2 + xyz[:, 1]**2
...
def virtual_scan(xyz, mode=-1):
    ...
    if mode == -1:
        view_dr = np.array([
            ...
            np.pi / 10 * (np.random.random() - 0.75)
        ])
        camloc[:2] -= (0.8 + 0.7 * np.random.random()) * np.array(
            [np.cos(view_dr[0]), np.sin(view_dr[0])])
    else:
        view_dr = np.array([np.pi / 4 * mode, 0])
        camloc[:2] -= np.array([np.cos(view_dr[0]), np.sin(view_dr[0])])
    ct_ray_dr = np.array([
        np.cos(view_dr[1]) * np.cos(view_dr[0]),
        np.cos(view_dr[1]) * np.sin(view_dr[0]),
        np.sin(view_dr[1])
    ])
    ...
    xx = xx.reshape(-1, 1)
    yy = yy.reshape(-1, 1)
    rays = xx * hr_dr.reshape(1, -1) + yy * vt_dr.reshape(
        1, -1) + ct_ray_dr.reshape(1, -1)
    rays_aer = cart2sph(rays)
    local_xyz = xyz - camloc.reshape(1, -1)
    local_aer = cart2sph(local_xyz)
    nbrs = NearestNeighbors(
        n_neighbors=1, algorithm='kd_tree').fit(rays_aer[:, :2])
