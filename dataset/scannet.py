...
class Dataset():
    ...
    def __init__(self, npoints, split, use_color, box_size, path, dropout_max,
        ...
        if use_color:
            print(
                "WARNING : no color available on scannet dataset. Setting use_color to false"
            )
        ...
        self.labels_names = [
            ...
            'counter', 'door', 'window', 'shower curtain', 'refrigerator',
    ...
    def __getitem__(self, index, visu=False):
        ...
        for _ in range(10):
            ...
            if len(cur_semantic_seg) == 0:
                continue
            mask = np.sum(
                (cur_point_set >= (curmin - 0.01)) * (cur_point_set <=
                                                      (curmax + 0.01)),
                axis=1) == 3
            vidx = np.ceil((cur_point_set[mask, :] - curmin) /
                           (curmax - curmin) * [31.0, 31.0, 62.0])
            vidx = np.unique(vidx[:, 0] * 31.0 * 62.0 + vidx[:, 1] * 62.0 +
                             vidx[:, 2])
            isvalid = np.sum(
                cur_semantic_seg > 0) / len(cur_semantic_seg) >= 0.7 and len(
                    vidx) / 31.0 / 31.0 / 62.0 >= 0.02
            if isvalid:
                break
        if visu:
            return index, curchoice, np.histogram(cur_semantic_seg,
                                                  range(22))[0], seed_idx
    ...
    def next_input(self, dropout=False, sample=True, verbose=False,
        ...
        if not sample:
            print(
                "WARNING : sampling in next_input of scannet is not a supported feature"
            )
        return self.__getitem__(
            np.random.randint(0, len(self.scene_points_list)), visu)

    def next_batch(self, batch_size, augment=False, dropout=False):
        if augment:
            print(
                "WARNING : augmentation in next_batch of scannet is not a supported feature"
            )
        if dropout:
            print(
                "WARNING : dropout in next_batch of scannet is not a supported feature"
            )
        batch_data = list()
        batch_label = list()
        batch_weights = list()
        for _ in range(batch_size):
            pt_set, sem_seg, smpw = self.next_input(dropout, True, False,
                                                    False)
